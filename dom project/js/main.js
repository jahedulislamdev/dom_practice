...
function loadMilestones () { 
  ...
  milestone.innerHTML =` ${ milestonesData.map(function(milestone) { 
    ...
    <div class="card-body bg-dark text-light"onclick="openMilestones(this) " >
          <div class="d-flex " >
                <div class="checkboxes">
                      <input type="checkbox" name="checkbox" id="checkbox">
                 </div>
                 <div>     
                      <p class="ml-2">${milestone.name} <span><i class="fa-solid fa-chevron-down"></i></span></p>
                </div>
          </div>
          <div class="hidden-panel"> 
                <p class="ml-1">${milestone.modules.map(function(module)  { 
                  return ` <p class="ml-1">${module.name}</p>`;
                }) .join("")} </p>
           </div>
    </div>
</div> ` ;
...
//Open milestone function
function openMilestones(milestoneElement){ 
const currentPanel =milestoneElement.parentNode.nextElementSibling;
 currentPanel.classList.toggle("show");
}
loadMilestones();